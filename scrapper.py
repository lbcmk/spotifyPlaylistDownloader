...
class bot:
    ...
    def get_tracks(self):
        total = requests.get("https://api.spotify.com/v1/playlists/0Hx4aQ2xTb4TKl5Z56DJh0/tracks?market=CA&fields=total", headers={"Content-Type": "application/json", "Authorization": f"Bearer {os.environ['API']}"}).json()['total']        
        for n in range(divmod(total, 100)[0] + 1):
            res = requests.get(f"https://api.spotify.com/v1/playlists/0Hx4aQ2xTb4TKl5Z56DJh0/tracks?market=CA&fields=items(track.name%2Ctrack.artists.name)&offset={n*100}", headers={"Content-Type": "application/json", "Authorization": f"Bearer {os.environ['API']}"}).json()    
            for i in res["items"]:
